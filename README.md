...
`[fim] watch_dirs`에 감시할 디렉토리를 콤마로 구분해 지정하면, `auditd`(커널 감사
서브시스템)로 그 안의 생성·삭제·내용변경·권한(소유자·모드)변경을 감지한다.
에이전트는 판정하지 않는다는 원칙 그대로 — auditd 워치 규칙만 걸어두고, 수집
주기마다 그 사이 쌓인 이벤트 원문(`ausearch -k kavis_fim -i`)만 서버로 넘기면
서버(`ingest.php`)가 CREATE/DELETE/CONTENT/SECURITY로 분류해 저장한다.
